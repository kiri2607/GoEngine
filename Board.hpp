...
struct Board{
    ...
    void find_group(PosList& buff, const Pos& pos, BitBoard& vis); // find a group of 'pos' and store its position in 'vis'
    void find_group(PosList& buff, const Pos& pos); // find a group of 'pos'
    ...
    void find_killed_groups(std::vector<PosList>& buff, const Pos& pos, bool col); // find all groups with color !col that were killed after placing a stone with color 'col' at 'pos'
    ...
    void get_with_air_count(PosList& buff, bool col, i8 cnt); // sorted list of spots of air of groups that have 'cnt' spots of air
    void get_pseudo_legal_moves(PosList& buff, bool col); // get all moves that are legal without the Ko rule
